...
class HorizontalCaptionAxis extends PureComponent{
    getCaptionLabels = ({x, px, labelWidth, labelHeight, key, axisWidth, xleft, xright }) => {
        ...
        if (x === xleft){
          return <LeftCaptionLabel onDragStart={onDragStart} px={px+50} y={labelHeight/2} xlabel={leftText} color={color.text} key={1} />
        }
        if (x === xright){
          return <RightCaptionLabel px={px-50} y={labelHeight/2} xlabel={rightText} color={color.text} key={2} />
        }      
      }
